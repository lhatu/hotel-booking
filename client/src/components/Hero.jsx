...
const Hero = () => {
    ...
    const onSearch = async (e) => {
        e.preventDefault();
        navigate(`/rooms?destination=${destionation}`);
        await axios.post('/api/users/store-recent-search', {recentSearchedCities: destionation},
            {header: { Authorization: `Bearer ${getToken()}` } }
        )
        setSearchedCities((prevSearchedCities) => {
            const updatedSearchedCities = [...prevSearchedCities, destionation];
        ...
        <div className='flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32 text-white bg-[url("/src/assets/heroImage.png")] bg-no-repeat bg-cover bg-center h-screen'>
            ...
            <form onSubmit={onSearch} className='bg-white text-gray-500 rounded-lg px-6 py-4 mt-8 flex flex-col md:flex-row max-md:items-start gap-4 max-md:mx-auto'>
                ...
                <div>
                    ...
                    <input onChange={e=>setDestination(e.target.value)} value={destionation} list='destinations' id="destinationInput" type="text"
